...
export function getOrderPeriod(order: Order): { startDate: Date; endDate: Date } {
  ...
  switch (order.order_number) {
    case 1: // Monday → Thursday
      return {
        startDate: addDays(deliveryDate, -3), // Friday previous week
        endDate: addDays(deliveryDate, 2), // Saturday
      }
    case 2: // Wednesday → Saturday
      return {
        startDate: addDays(deliveryDate, -3), // Monday
        endDate: addDays(deliveryDate, 3), // Tuesday next week
      }
    case 3: // Friday → Tuesday
      return {
        startDate: addDays(deliveryDate, -4), // Wednesday
        endDate: addDays(deliveryDate, 2), // Thursday next week
      }
    default:
...
export function calculateOrderNeeds(
  ...
  const stock = order.real_stock?.[product.id!] || 
  ...
  // Allow negative needs (excess stock)
